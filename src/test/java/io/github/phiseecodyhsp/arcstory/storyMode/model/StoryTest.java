...
class StoryTest {
    ...
    void deserialize_parsesSnakeCaseJson() throws Exception {
        ...
        assertEquals(ParagraphType.TEXT, story.getParagraphs().get(0).type());
        assertEquals("texts", story.getParagraphs().get(0).location().category());
        assertEquals("test_text", story.getParagraphs().get(0).location().key());
